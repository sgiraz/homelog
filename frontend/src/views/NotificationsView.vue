  ...
  <div class="max-w-3xl mx-auto px-4 py-6">
    ...
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center gap-3">
        <button
          @click="$router.back()"
          class="p-2 -ml-2 rounded-lg text-ink-muted hover:bg-surface-2 transition-colors"
        >
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
          </svg>
        </button>
        <h1 class="text-2xl font-bold text-ink">{{ t('notifications.title') }}</h1>
      </div>
    ...
    <div v-if="loading" class="space-y-3">
      <div v-for="i in 5" :key="i" class="bg-surface rounded-xl p-4 animate-pulse">
        <div class="flex items-start gap-3">
          <div class="w-9 h-9 bg-surface-3 rounded-lg" />
          <div class="flex-1 space-y-2">
            <div class="h-4 bg-surface-3 rounded w-1/3" />
            <div class="h-3 bg-surface-3 rounded w-2/3" />
            <div class="h-3 bg-surface-3 rounded w-1/4" />
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="filteredNotifications.length === 0" class="text-center py-16">
      ...
      <p class="text-ink-muted text-lg font-medium">
    ...
    <div v-else class="space-y-2">
      <TransitionGroup name="list">
        <div
          ...
          class="bg-surface rounded-xl border border-line
          ...
          <div class="flex items-start gap-3 p-4">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-center gap-3 mt-2">
                ...
                <span
                  v-if="notif.type"
                  class="px-1.5 py-0.5 text-[10px] font-medium rounded bg-surface-2 text-ink-muted"
                >
